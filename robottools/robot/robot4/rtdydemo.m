...
% state of joint position, velocity and acceleration.  The recursive Newton-Euler
% formulation is an efficient matrix oriented algorithm for computing the inverse
% dynamics, and is implemented in the function rne().
%
%  Inverse dynamics requires inertial and mass parameters of each link, as well
% as the kinematic parameters.  This is achieved by augmenting the kinematic description
% matrix with additional columns for the inertial and mass parameters for each link.
%
...
%  As with other functions the inverse dynamics can be computed for each point on
% a trajectory.  Create a joint coordinate trajectory and compute velocity and
...
% Now lets compare the gravity torque with the total torque required over the trajectory
	subplot(2,1,1)
	plot(t,[tau(:,2) taug(:,2)])
	xlabel('Time (s)');
...
%
%  Let's compute the variation in joint 1 inertia, that is M(1,1), as the manipulator
...
% Clearly the inertia seen by joint 1 varies considerably over this path.  This is 
% one of many challenges to control design in robotics, achieving stability and 
% high-performance in the face of plant variation.  In fact for this example the 
% inertia varies by a factor of
	max(M11)/min(M11)
pause	% any key to continue
